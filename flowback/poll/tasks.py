...
def poll_prediction_bet_count(poll_id: int):
    ...
    outcome_subquery = PollPredictionStatementVote.objects.filter(prediction_statement_id=OuterRef('id'),
                                                                  ...
                                                                  ).aggregate(outcome=Sum('vote')
                                                                              ).values('outcome')

    # Get list of previous outcomes in a given area (poll)
    outcomes = PollPredictionStatement.objects.filter(poll__tag=poll.tag,
    ...
    for predictor in predictors:
        bet_subquery = PollPredictionBet.objects.filter(created_by=predictor,
                                                        prediction_statement_id=OuterRef('id')).values('score')

        bets = PollPredictionStatement.objects.filter(poll__tag=poll.tag,
                                                      poll__end_date__lte=timestamp,
                                                      created_at__lte=timestamp
                                                      ).annotate(bet=Subquery(bet_subquery)
                                                                 ).values_list('bet_subquery', flat=True)
    ...
    predictor_bets = [[0, 1, 0.7, 0, 1], [0, 0.2, 1, 0, 0.8]]

    # Calculation below

    predictor_errors = []
    for bets in predictor_bets:
        predictor_errors.append(np.array([outcomes[i] - bets[i]
                                          if bets[i] is not None else None for i in range(len(outcomes))]))
    ...
    combined_bet = float(np.matmul(transposed_bet_weights, current_bets)[0][0])
